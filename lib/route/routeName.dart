const String initRoute = "/";
const String dasAplhabet = "dasalphabet";
const String dieBegruBungen = "diebegrüßungen";
const String sichvorstellen = 'sichvorstellen';
const String dieArbeit = 'diearbeit';
const String aufDemMarkt = 'aufdemmarkt';
const String dieZeit = 'diezeit';
const String dasEssenUndTrinken = 'dasessenundtrinken';

const String ListChapitresa1 = "chapitresA1";
const String diefamilie = 'diefamilie';
const String BaatYiRoute = 'baat_yi';
